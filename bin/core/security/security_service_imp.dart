...
class SecurityServiceImp implements SecurityService<JWT> {
  SecurityServiceImp(param0);

  @override
  Future<String> generateJWT(String document, String roles) async {
    var jwt = JWT({
      'iat': DateTime.now().millisecondsSinceEpoch,
      'userID': document,
      'roles': [roles]
    });
    String key = await CustonEnv.get(key: 'jwt_key');
  ...
  Middleware get verifyJwt => createMiddleware(requestHandler: (Request req) {
        if (req.context['jwt'] == null) {
          return Response.forbidden('Not Authorization');
        }
        return null;
      });
}
